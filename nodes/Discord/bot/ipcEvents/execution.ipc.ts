    ...
    try {
      ...
      if (data.executionId && data.channelId) {
        ...
        if (data.placeholderId && data.apiKey && data.baseUrl) {
          if (!state.placeholderMatching.has(data.placeholderId)) {
            state.placeholderMatching.set(data.placeholderId, data.placeholderId)
          }
              ...
              .then((res) => {
                ...
                if (res?.data?.finished === false && res.data.stoppedAt === null) {
                  ...
                  const timeout = setTimeout(() => {
                    if (state.placeholderMatching.has(placeholderId)) {
