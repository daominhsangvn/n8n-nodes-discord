...
export default async function (token: string, clientId: string, client: Client) {
  // Resolve all command imports
  const resolvedCommands = await Promise.all(awaitingCommands)
  ...
  client.on('interactionCreate', async (interaction: Interaction) => {
    try {
      ...
      const command = resolvedCommands[i].default

      // Execute the command
      const reply = await command
        .executeCommand(options.get('input')?.value as string, interaction)
